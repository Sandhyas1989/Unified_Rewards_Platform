...
{
    private readonly string _dbName = $"test-pi-{Guid.NewGuid():N}";
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        builder.ConfigureServices(services =>
        {
            var desc = services.SingleOrDefault(
                d => d.ServiceType == typeof(DbContextOptions<PayrollDbContext>));
            if (desc is not null) services.Remove(desc);
            services.AddDbContext<PayrollDbContext>(o =>
                o.UseSqlite($"Data Source={_dbName};Mode=Memory;Cache=Shared"));

            // Stop the settlement worker and event-subscriber background services
