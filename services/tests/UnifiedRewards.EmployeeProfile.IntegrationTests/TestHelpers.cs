...
{
    public Task PublishAsync<T>(T @event, Guid tenantId, CancellationToken ct = default) where T : class
        => Task.CompletedTask;
}

// JWT tokens signed with the same RS256 key loaded by the service under test.
// Issuer/audience validation is disabled in the test fixture, so only the signature matters.
...
// One fresh in-memory SQLite per fixture instance → isolated per test class.
// The SqliteConnection is kept open so the named in-memory DB survives between requests.
public sealed class ServiceFixture : WebApplicationFactory<Program>
{
    private readonly string _dbName = $"test-ep-{Guid.NewGuid():N}";
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        builder.ConfigureServices(services =>
        {
            var desc = services.SingleOrDefault(
                d => d.ServiceType == typeof(DbContextOptions<EmployeeProfileDbContext>));
            if (desc is not null) services.Remove(desc);
            services.AddDbContext<EmployeeProfileDbContext>(o =>
                o.UseSqlite($"Data Source={_dbName};Mode=Memory;Cache=Shared"));

            // Swap event bus with no-op and suppress outbox dispatcher background service.
            services.RemoveAll<IEventBus>();
            services.AddScoped<IEventBus, NullEventBus>();
            services.RemoveAll<IHostedService>();

            // Disable issuer/audience in tests — only the RS256 signature is verified.
