...
// Returns a fixed storage reference for any upload; no real file I/O in tests.
sealed class StubFileStorage : IFileStorage
{
    public Task<string> UploadAsync(string fileName, Stream stream, string contentType, CancellationToken ct = default)
        => Task.FromResult("stub-storage-ref");

    public Task<Stream> DownloadAsync(string reference, CancellationToken ct = default)
        => Task.FromResult<Stream>(new MemoryStream("stub-content"u8.ToArray()));
}

// Returns a fixed OCR result; tests don't rely on real Tesseract/Doc-Intelligence.
sealed class StubOcrEngine : IOcrEngine
{
    public Task<OcrResult> ExtractAsync(byte[] bytes, CancellationToken ct = default)
        => Task.FromResult(new OcrResult("Test receipt", 0.95m, 250.00m));
}

// No-op event bus — integration tests exercise the HTTP/DB layer; messaging is covered by EndToEndTests.
...
{
    private readonly string _dbName = $"test-dp-{Guid.NewGuid():N}";
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        builder.ConfigureServices(services =>
        {
            ...
            // Replace external-I/O dependencies with stubs
            ...
            // Don't run outbox-dispatcher background service in tests
